...
public class GetSections extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String xmlDbSectionsPath = getServletContext().getRealPath(
                            SysKb.xmlDbSections);
        if(!(new File(xmlDbSectionsPath).exists())){
            //Se non esiste il file è un errore
            sendError("Impossibile trovare le sezioni al momento", out);
            return;
        }
        //Recupero il file e lo ritorno in output come response
        WebUtils.fileToOutputStream(xmlDbSectionsPath, out);
    }
